...
(function ($) {
  ...
  Canvas.prototype.resize = function (width, height) {
    ...
    if (width <= 0 || height <= 0) {
      throw new Error(
          "Invalid dimensions for plot, width = " + width + ", height = "
          + height);
    }
  ...
  Canvas.prototype.getTextInfo = function (layer, text, font, angle, width) {
    ...
    if (typeof font === "object") {
      textStyle = font.style + " " + font.variant + " " + font.weight + " "
          + font.size + "px/" + font.lineHeight + "px " + font.family;
    } else {
      textStyle = font;
    }
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function parseOptions(opts) {
      ...
      if (options.xaxis.color == null) {
        options.xaxis.color = $.color.parse(options.grid.color).scale('a',
            0.22).toString();
      }
      if (options.yaxis.color == null) {
        options.yaxis.color = $.color.parse(options.grid.color).scale('a',
            0.22).toString();
      }
      ...
      for (i = 0; i < series.length; ++i) {
        ...
        for (j = 0; j < points.length; j += ps) {
          ...
          for (m = 0; m < ps; ++m) {
            val = points[j + m];
            f = format[m];
            if (!f || f.autoscale === false || val == fakeInfinity || val
                == -fakeInfinity) {
              continue;
            }
    ...
    function measureTickLabels(axis) {
      ...
      var opts = axis.options,
          ...
          maxWidth = labelWidth || axis.direction == "x" ? Math.floor(
              surface.width / (ticks.length || 1)) : null,
          legacyStyles = axis.direction + "Axis " + axis.direction + axis.n
              + "Axis",
          layer = "flot-" + axis.direction + "-axis flot-" + axis.direction
              + axis.n + "-axis " + legacyStyles,
    ...
    function allocateAxisBoxSecondPhase(axis) {
      ...
      if (axis.direction == "x") {
        ...
        axis.box.width = surface.width - plotOffset.left - plotOffset.right
            + axis.labelWidth;
      }
      else {
        axis.box.top = plotOffset.top - axis.labelHeight / 2;
        axis.box.height = surface.height - plotOffset.bottom - plotOffset.top
            + axis.labelHeight;
      }
    }
    ...
    function adjustLayoutForThingsStickingOut() {
      ...
      $.each(allAxes(), function (_, axis) {
        ...
        if (axis.reserveSpace) {
          margins[dir] = Math.ceil(Math.max(margins[dir],
      ...
      for (var j = 0; j < axes.length; ++j) {
        ...
        for (i = 0; i < axis.ticks.length; ++i) {
          ...
          if (isNaN(v) || v < axis.min || v > axis.max
              ...
              || (t == "full"
                  && ((typeof bw == "object" && bw[axis.position] > 0) || bw
                      > 0)
    ...
    function drawAxisLabels() {
      ...
      $.each(allAxes(), function (_, axis) {
        ...
        for (var i = 0; i < axis.ticks.length; ++i) {
          ...
          surface.addText(layer, x, y, tick.label, font, null, null, halign,
              valign);
        }
      });
    }
    ...
    function drawPointHighlight(series, point) {
      var x = point[0], y = point[1],
          ...
          highlightColor = (typeof series.highlightColor === "string")
              ? series.highlightColor : $.color.parse(series.color).scale('a',
                  0.5).toString();

...
// Add gradient to IE
  setTimeout(function () {
    $("input, textarea, .select_styled, .boxed, .widget_categories li a, .comment-body, .price_col_head, .price_body_top, .blog_filter ul, .ico-cat, .tabs li, .tabs li span, .tab-content, .counter, .ui-datepicker-header, .dropdown > li a, .tabs li a, .tab-pane, .comment-body .inner, .carouFredsel.style2, .dropdownMenu a, .carousel-title").addClass(
        "gradient");
  }, 0);
...
// Payment Form
  $('.payment-form #billing .btn-next, .payment-form #payment .btn-prev').click(
      function () {
...
// Audio Player
  ...
  if ($players_on_page > 0) {
    ...
    function switchSong() {
      setTimeout(function () {
        for (var i = 1; i <= $players_on_page; i++) {
          $('.jp-audio' + i + ' .jp-previous, .jp-audio' + i
    ...
    setTimeout(function () {
      for (var i = 1; i <= carousels_on_page; i++) {
        ...
        for (var j = 1; j <= images_in_carousel; j++) {
          ...
          if (ratio <= imgRatio) {
            imgMargin = parseInt(
                (carouselHeight / imgHeight * imgWidth - carouselWidth) / 2,
                10);
            this_image.css("cssText",
                "height: " + carouselHeight + "px; margin-left:-" + imgMargin
                + "px;");
          }
          else {
            imgMargin = parseInt(
                (carouselWidth / imgWidth * imgHeight - carouselHeight) / 2,
                10);
            this_image.css("cssText",
                "width: " + carouselWidth + "px; margin-top:-" + imgMargin
                + "px;");
          }
        }
      }

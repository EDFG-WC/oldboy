...
var bkElement = bkClass.extend({
  ...
  pos: function () {
    ...
    var b = (!window.opera) ? parseInt(
        this.getStyle('border-width') || this.style.border) || 0 : 0;
  ...
  getStyle: function (cssRule, d) {
    ...
    if (this.nodeType == 1) {
      return (doc && doc.getComputedStyle) ? doc.getComputedStyle(this,
          null).getPropertyValue(cssRule) : this.currentStyle[bkLib.camelize(
          cssRule)];
    }
  },
...
var bkLib = {
  ...
  onDomLoaded: function (fireThis) {
    ...
    if (document.addEventListener) {
      ...
      document.write("<style>.nicEdit-main p { margin: 0; }</style><scr"
          + "ipt id=__ie_onload defer " + ((location.protocol == "https:")
              ? "src='javascript:void(0)'" : "src=//0") + "><\/scr" + "ipt>");
...
var nicEditorConfig = bkClass.extend({
  ...
  buttonList: ['save', 'bold', 'italic', 'underline', 'left', 'center', 'right',
    'justify', 'ol', 'ul', 'fontSize', 'fontFamily', 'fontFormat', 'indent',
    'outdent', 'image', 'upload', 'link', 'unlink', 'forecolor', 'bgcolor'],
  iconList: {
    "xhtml": 1,
    "bgcolor": 2,
    "forecolor": 3,
  ...
  allTextAreas: function (nicOptions) {
    ...
    for (var i = 0; i < textareas.length; i++) {
      nicEditors.editors.push(
          new nicEditor(nicOptions).panelInstance(textareas[i]));
    }
...
var nicEditorInstance = bkClass.extend({
  ...
  construct: function (e, options, nicEditor) {
    ...
    if (isTextarea || this.options.hasPanel) {
      var ie7s = (bkLib.isMSIE && !((typeof document.body.style.maxHeight
      ...
      this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(
  ...
  init: function () {
    ...
    this.elm.addEvent('mousedown',
        ...
        this.blur.closure(this)).addEvent('keyup', this.selected.closure(this));
  ...
  selElm: function () {
    ...
    if (r.startContainer) {
      ...
      return $BK(
          (this.getSel().type == "Control") ? r.item(0) : r.parentElement());
    }
  },
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
  ...
  init: function () {
    ...
    this.elmFrame = new bkElement('iframe').setAttributes({
      ...
      'scrolling': 'no'
    }).setStyle({height: '100px', width: '100%'}).addClass('frame').appendTo(
        this.elm);
  ...
  heightUpdate: function () {
    this.elmFrame.style.height = Math.max(this.frameContent.offsetHeight,
...
var nicEditorPanel = bkClass.extend({
  construct: function (e, options, nicEditor) {
    ...
    this.panelContain = new bkElement('DIV').setStyle(
        {overflow: 'hidden', width: '100%'}).addClass('panelContain');
    this.panelElm = new bkElement('DIV').setStyle(
        {zoom: 1, overflow: 'hidden'}).addClass('panel').appendTo(
        this.panelContain);
  ...
  addButton: function (buttonName, options, noOrder) {
    ...
    var type = (button['type']) ? eval(
        '(typeof(' + button['type'] + ') == "undefined") ? null : '
        + button['type'] + ';') : nicEditorButton;
...
var nicEditorButton = bkClass.extend({
  ...
  construct: function (e, buttonName, options, nicEditor) {
    ...
    this.contain = new bkElement('DIV').setStyle(
        {width: '30px', height: '26px'}).addClass('buttonContain').appendTo(
        this.margin);
    ...
    this.button = new bkElement('DIV').setStyle({
      ...
      zoom: 1,
      cursor: 'pointer'
    }).addClass('button').setStyle(
        this.ne.getIcon(buttonName, options)).appendTo(this.border);
    this.button.addEvent('mouseover', this.hoverOn.closure(this)).addEvent(
        'mouseout', this.hoverOff.closure(this)).addEvent('mousedown',
    ...
    nicEditor.addEvent('selected', this.enable.closure(this)).addEvent('blur',
  ...
  key: function (nicInstance, e) {
    if (this.options.key && e.ctrlKey && String.fromCharCode(
        e.keyCode || e.charCode).toLowerCase() == this.options.key) {
      this.mouseClick();
      if (e.preventDefault) {
        e.preventDefault();
      }
    ...
    if (openButton && !openButton.options.noClose) {
      this.close = new bkElement('div').setStyle({
        ...
        cursor: 'pointer'
          ...
          'mousedown', openButton.removePane.closure(this)).appendTo(this.pane);
    }
...
var nicEditorAdvancedButton = nicEditorButton.extend({
  ...
  init: function () {
    this.ne.addEvent('selected', this.removePane.closure(this)).addEvent('blur',
    ...
    if (!this.isDisabled) {
      if (this.pane && this.pane.pane) {
        ...
        this.pane = new nicEditorPane(this.contain, this.ne,
            {width: (this.width || '270px'), backgroundColor: '#fff'}, this);
        this.addPane();
    ...
    for (itm in f) {
      ...
      if (type == 'title') {
        ...
        var contain = new bkElement('div').setStyle(
            {overflow: 'hidden', clear: 'both'}).appendTo(this.form);
        if (field.txt) {
          new bkElement('label').setAttributes({'for': itm}).setContent(
              field.txt).setStyle({
            margin: '2px 4px',
            fontSize: '13px',
            width: '50px',
        ...
        switch (type) {
          case 'text':
            this.inputs[itm] = new bkElement('input').setAttributes(
                {id: itm, 'value': val, 'type': 'text'}).setStyle({
              margin: '2px 0',
              fontSize: '13px',
              'float': 'left',
              height: '20px',
          ...
          case 'content':
            this.inputs[itm] = new bkElement('textarea').setAttributes(
                {id: itm}).setStyle(
                {border: '1px solid #ccc', 'float': 'left'}).setStyle(
                field.style).appendTo(contain);
...
var nicButtonTips = bkClass.extend({
  construct: function (nicEditor) {
    this.ne = nicEditor;
    nicEditor.addEvent('buttonOver', this.show.closure(this)).addEvent(
        'buttonOut', this.hide.closure(this));
...
var nicEditorSelect = bkClass.extend({
  ...
  construct: function (e, buttonName, options, nicEditor) {
    ...
    this.margin = new bkElement('div').setStyle(
        {'float': 'left', margin: '2px 1px 0 1px'}).appendTo(this.elm);
    this.contain = new bkElement('div').setStyle({
      width: '90px',
      height: '20px',
      cursor: 'pointer',
    ...
    this.control = new bkElement('div').setStyle({
      ...
      width: '16px'
        ...
        this.ne.getIcon('arrow', options)).appendTo(this.items);
    ...
    this.ne.addEvent('selected', this.enable.closure(this)).addEvent('blur',
  ...
  open: function () {
    ...
    for (var i = 0; i < this.selOptions.length; i++) {
      ...
      var itm = new bkElement('div').setStyle({padding: '0px 4px'}).setContent(
          opt[1]).appendTo(itmContain).noSelect();
      itm.addEvent('click', this.update.closure(this, opt[0])).addEvent(
          'mouseover', this.over.closure(this, itm)).addEvent('mouseout',
          this.out.closure(this, itm)).setAttributes('id', opt[0]);
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({
  ...
  init: function () {
    ...
    for (itm in this.sel) {
      ...
      this.add('<' + tag + '>',
          '<' + itm + ' style="padding: 0px; margin: 0px;">' + this.sel[itm]
          + '</' + tag + '>');
    }
  }
});
      ...
      for (var b in colorList) {
        for (var g in colorList) {
          ...
          var colorBorder = new bkElement('DIV').setStyle(
              {border: '2px solid ' + colorCode}).appendTo(colorSquare);
          var colorInner = new bkElement('DIV').setStyle({
            ...
            height: '11px'
              ...
              this.colorSelect.closure(this, colorCode)).addEvent('mouseover',
              this.on.closure(this, colorBorder)).addEvent('mouseout',
              this.off.closure(this, colorBorder, colorCode)).appendTo(
              colorBorder);
      ...
      case 1:
        ...
        if (r) {
          for (var i = 0; i < nAttributes.length; i++) {
            ...
            if (!attr.specified || !attributeValue || bkLib.inArray(
                this.stripAttributes, attributeName) || typeof(attributeValue)
                == "function") {
              continue;
            }

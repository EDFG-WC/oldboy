  ...
  jQuery.extend({
    valHooks: {
      ...
      select: {
        ...
        set: function (elem, value) {
          ...
          while (i--) {
            option = options[i];
            if ((option.selected = jQuery.inArray(jQuery(option).val(), values)
                >= 0)) {
              optionSet = true;
            }
          }
    ...
    attr: function (elem, name, value) {
      ...
      if (value !== undefined) {
        ...
        } else if (hooks && "set" in hooks && (ret = hooks.set(elem, value,
            name)) !== undefined) {
          return ret;
    ...
    prop: function (elem, name, value) {
      ...
      if (value !== undefined) {
        return hooks && "set" in hooks && (ret = hooks.set(elem, value, name))
        !== undefined ?
            ret :
            (elem[name] = value);
  ...
  jQuery.event = {
    ...
    trigger: function (event, data, elem, onlyHandlers) {
      ...
      if (!onlyHandlers && !event.isDefaultPrevented()) {
        ...
        if ((!special._default || special._default.apply(eventPath.pop(), data)
            === false) &&
...
// because jsdom on node.js will break without it.
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    if (val <= 0 || val == null) {
      ...
      valueIsBorderBox = isBorderBox && (jQuery.support.boxSizingReliable || val
          === elem.style[name]);
...
// Attach a bunch of functions for handling common AJAX events
  ...
  jQuery.extend({
    ...
    ajax: function (url, options) {
      ...
      s.url = ((url || s.url || ajaxLocation) + "").replace(rhash, "").replace(

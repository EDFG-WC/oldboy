jQuery.fn.anim_progressbar = function (aOptions) {
  ...
  aOpts.finish = new Date().setTime(
      new Date().getTime() + aOpts.totaltime * iSms);
  var vPb = this;
        ...
        var vInterval = setInterval(
            function () {
              ...
              var iElapsedMs = new Date() - aOpts.start, // elapsed time in MS
                  ...
                  iMin = parseInt(
                      (iLeftMs - (iDays * iDms) - (iHours * iHms)) / iMms), // remained minutes
                  iSec = parseInt(
                      (iLeftMs - (iDays * iDms) - (iMin * iMms) - (iHours
                          * iHms)) / iSms), // remained seconds

                  eDays = parseInt(iElapsedMs / iDms), // elapsed days
                  eHours = parseInt((iElapsedMs - (eDays * iDms)) / iHms), // elapsed hours
                  eMin = parseInt(
                      (iElapsedMs - (eDays * iDms) - (eHours * iHms)) / iMms), // elapsed minutes
                  eSec = parseInt(
                      (iElapsedMs - (eDays * iDms) - (eMin * iMms) - (eHours
                          * iHms)) / iSms), // elapsed seconds
              ...
              if (eHours == 0) {
                ...
                $(vPb).children('.remained').html(
                    eHours + ':' + eMin + ':' + eSec);
              }
              if (iPerc >= 100) {
                iPerc = 100
              }
              $(vPb).children('.pbar').children('.ui-progressbar-value').css(
                  'width', iPerc + '%');
              if (iPerc < 4) {
                iPerc = 4;
              }

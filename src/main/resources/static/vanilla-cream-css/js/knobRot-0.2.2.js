  ...
  var methods = {
    ...
    init: function (options) {
      ...
      return this.each(function () {
        ...
        if ($this.is('input:text')) {
          ...
          realValueField.data('knobRot').rangeSize = settings.maximumValue
              - settings.minimumValue;
          realValueField.data('knobRot').rangeOffset = 0
              - settings.minimumValue;

          //Determine the step increment amount
          if (settings.discreteSteps == true) {
            realValueField.data(
                'knobRot').stepIncrement = (settings.maximumValue
                - settings.minimumValue) / (settings.stepCount - 1);
          }

          //Calculate the initial value
          realValueField.data(
              'knobRot').calculatedValue = methods.calculateValue(
              realValueField);

          //Calculate the initial frame offset
          realValueField.data('knobRot').currentFrame = methods.calculateFrame(
              realValueField);
          ...
          knobDiv.css({
            ...
            'background-position': methods.calculateBackgroundOffsetX(
                realValueField) + 'px 0px',
          ...
          knobDiv.on('mousedown.knobRot', function (event) {
            ...
            if ($('body').data('knobRot').dragging != true) {
              ...
              // Set the drag cursor
              $('body').addClass(methods.getDragCursorClass(
                  $knobDiv.data('knobRot').realValue));

              // Trigger some events
              $knobDiv.data('knobRot').outputField.trigger('knobdragstart',
                  $knobDiv);
            }
          });
          ...
          $(document).on('mousemove.knobRot', function (event) {
            ...
            if ($('body').data('knobRot').dragging == true) {
              ...
              var displacement = {
                'horizontal': event.screenX - $('body').data(
                    'knobRot').lastOffset.left,
                'vertical': event.screenY - $('body').data(
                    'knobRot').lastOffset.top
              }
              ...
              methods.updateValue(
                  $('body').data('knobRot').knobDiv.data('knobRot').realValue,
                  displacement);

              $('body').data('knobRot').knobDiv.data(
                  'knobRot').outputField.trigger('knobrefresh',
                  $('body').data('knobRot').knobDiv);
              $('body').data('knobRot').knobDiv.data(
                  'knobRot').outputField.trigger('knobdrag',
                  $('body').data('knobRot').knobDiv);
            }
          });
          ...
          $('body').on(
              'selectstart.knobRot select.knobRot mousedown.knobRot mouseover.knobRot',
              function () {
                if ($('body').data('knobRot').dragging == true) {
                  return false;
                }
          ...
          knobDiv.on('mouseover.knobRot mouseout.knobRot mouseup.knobRot',
              function () {
          ...
          knobDiv.on('mouseover', function () {
            $this.addClass('hover');
            var cursorClass = methods.getDragCursorClass(
                knobDiv.data('knobRot').realValue);
            knobDiv.addClass(cursorClass);
            // Trigger some events
            knobDiv.data('knobRot').outputField.trigger('knobmouseover',
                knobDiv);
          });
          knobDiv.on('mouseout', function () {
            ...
            knobDiv.data('knobRot').outputField.trigger('knobmouseout',
                knobDiv);
          });
    ...
    calculateStep: function ($realValueField) {
      ...
      if (knobSettings.discreteSteps) {
        ...
        var adjustedValue = parseFloat($realValueField.val())
            + knobData.rangeOffset;
        ...
        var calculatedStep = Math.round(
            rangeFraction * (knobSettings.stepCount - 1));
    ...
    calculateValue: function ($realValueField) {
      ...
      if (knobSettings.discreteSteps == true) {
        ...
        var calculatedValue = currentStep * knobData.stepIncrement
            - knobData.rangeOffset;
      ...
      if (knobSettings.detent == true && calculatedValue
          >= knobSettings.detentValue - knobSettings.detentThreshold
          && calculatedValue <= knobSettings.detentValue
          + knobSettings.detentThreshold) {
        ...
        if ($realValueField.data('knobRot').calculatedValue != calculatedValue
          ...
          //Trigger an event
          $realValueField.data('knobRot').outputField.trigger('knobenterdetent',
              $realValueField.data('knobRot').knobDiv);
        }

        //Detent the value
        calculatedValue = knobSettings.detentValue;

      } else if ($realValueField.data('knobRot').calculatedValue
          == knobSettings.detentValue
          && $realValueField.data('knobRot').calculatedValue != calculatedValue
          && calculatedValue != knobSettings.detentValue) {

        //Trigger an event
        $realValueField.data('knobRot').outputField.trigger('knobleavedetent',
            $realValueField.data('knobRot').knobDiv);
      }
      ...
      if (calculatedValue < knobSettings.minimumValue) {
        ...
        //Trigger an event
        $realValueField.data('knobRot').outputField.trigger('knobvaluemin',
            $realValueField.data('knobRot').knobDiv);
        ...
        //Trigger an event
        $realValueField.data('knobRot').outputField.trigger('knobvaluemax',
            $realValueField.data('knobRot').knobDiv);
    ...
    calculateFrame: function ($realValueField) {
      ...
      var calculatedValue = methods.calculateValue($realValueField)
          + knobData.rangeOffset;
      ...
      //Work out the frame
      var calculatedFrame = Math.round(
          rangeFraction * (knobSettings.frameCount - 1));
      ...
      if ($realValueField.data('knobRot').currentFrame != calculatedFrame) {
        $realValueField.data('knobRot').outputField.trigger('knobframechange',
            [knobData, $realValueField.data('knobRot').currentFrame,
              calculatedFrame]);
      }
    ...
    calculateBackgroundOffsetX: function ($realValueField) {
      var knobData = $realValueField.data('knobRot');
      var knobSettings = knobData.settings;
      return 0 - methods.calculateFrame($realValueField)
          * knobSettings.frameWidth;
    },
    /**
     * Animation callback
     */
    updateCallback: function ($realValueField) {
      ...
      if ($realValueField.data('knobRot').dirtyData == true) {
        ...
        $realValueField.data('knobRot').knobDiv.css('background-position',
            methods.calculateBackrgroundOffset($realValueField));
        $realValueField.data(
            'knobRot').calculatedValue = methods.calculateValue(
            $realValueField);
        $realValueField.data('knobRot').outputField.val(
            $realValueField.data('knobRot').calculatedValue);
        $realValueField.data('knobRot').outputField.data(
            'knobRot').calculatedValue = $realValueField.data(
            'knobRot').calculatedValue;
      }
    },
    /**
    ...
    stopDrag: function () {
      ...
      if ($('body').data('knobRot').dragging == true) {
        ...
        //Set the drag cursor
        $('body').removeClass(
            methods.getDragCursorClass($knobDiv.data('knobRot').realValue));
    ...
    updateValue: function ($realValueField, displacement) {
      ...
      if ((!$realValueField.data('knobRot').settings.invertDirection
          && $realValueField.data('knobRot').settings.dragVertical == true)
          || ($realValueField.data('knobRot').settings.invertDirection
              && $realValueField.data('knobRot').settings.dragVertical
              == false)) {
        change = 0 - change;
      }
      ...
      } else if (newValue < $realValueField.data(
          'knobRot').settings.minimumValue) {
    ...
    calculateBackrgroundOffset: function ($realValueField) {
      ...
      if ($realValueField.data('knobRot').outputField.hasClass('hover') && $(
          'body').data('knobRot').dragging == false) {
        offsetY = (0 - $realValueField.data('knobRot').settings.frameHeight)
            + "px";
      }

      //If we're dragging and we're dragging the right element, select the dragging graphics
      if ($('body').data('knobRot').dragging == true && $realValueField.is(
          $('body').data('knobRot').knobDiv.data('knobRot').realValue)) {
        offsetY = (0 - $realValueField.data('knobRot').settings.frameHeight * 2)
            + "px";
      }
